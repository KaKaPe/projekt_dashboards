...
def tab1_choropleth_sales(start_date, end_date):
    ...
    data = [trace0]
    fig = go.Figure(data=data, layout=go.Layout(title='Mapa',
...
# tab3 callbacks
@app.callback(Output('violin-customer-demography', 'figure'),
              [Input('store-type-dropdown', 'value')])
def tab3_violin_customer_demography(store_type):

    df.merged['DOB'] = pd.to_datetime(df.merged['DOB'], format='%d-%m-%Y')
    df.merged['age'] = df.merged['DOB'].apply(lambda x: dt.date.today().year - x.year)
    filtered = df.merged[(df.merged['total_amt'] > 0) & (df.merged['Store_type'] == store_type)]

    fig = go.Figure(layout=go.Layout(title='Demografia Klientów'))

    fig.add_trace(go.Violin(y=filtered['age'][filtered["Gender"] == 'M'],
                            legendgroup='M', scalegroup='M', name='M',
                            line_color='blue'))
    fig.add_trace(go.Violin(y=filtered['age'][filtered["Gender"] == 'F'],
                            legendgroup='F', scalegroup='F', name='F',
                            line_color='orange'))

    fig.update_traces(meanline_visible=True)
    fig.update_layout(xaxis_title="Gender", yaxis_title="Age", violinmode='overlay', showlegend=False)
